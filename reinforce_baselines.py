...
class WarmupBaseline(Baseline):
    ...
    def eval(self, x, graph, c):
        ...
        if self.alpha == 1:
            return self.baseline.eval(x, graph, c)
        if self.alpha == 0:
            return self.warmup_baseline.eval(x, graph, c)
        v, l = self.baseline.eval(x, c)
        vw, lw = self.warmup_baseline.eval(x, graph, c)
...
class RolloutBaseline(Baseline):
    ...
    def wrap_dataset(self, dataset):
        print("\nEvaluating + wrapping Rollout baseline on dataset...")
